...
const Climb= () => {
  ...
  const handleBack=async()=>{
    navigate(-1);
    const res = await axios.get('http://localhost:5000/pose/stop');
  ...
  const handleMonitorClick = async () => {
    ...
    const response = await axios.post("http://localhost:5000/pose/selectFactory",
      {
      ...
      await axios.get('http://localhost:5000/pose/start')
          ...
          <div className="p-8">
            ...
            {isMonitoring && (
               ...
               <div className="relative bg-gray-200 rounded-lg shadow-md w-[643px] h-[482px]" style={{ paddingTop: "56.25%" }}>
                 <iframe
                   src="http://localhost:5000/pose/video"
